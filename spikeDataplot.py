...
# collect the TOCs
    ...
    spikeData = file[wellID + '/SpikeData'][spikeStartPosition:spikeEndPosition]
    spikeDataLength = len(spikeData)
    pos = 0
    while pos < spikeDataLength:
        chIdx = int.from_bytes(spikeData[pos:pos + 4], byteorder='little', signed=True)
        pos += 4
        chDataLength = int.from_bytes(spikeData[pos:pos + 4], byteorder='little', signed=True)
        pos += 4
        chDataPos = pos
    while pos < chDataPos + chDataLength:
        fromInclusive = int.from_bytes(spikeData[pos:pos + 8], byteorder='little', signed=True)
        pos += 8
        toExclusive = int.from_bytes(spikeData[pos:pos + 8], byteorder='little', signed=True)
        pos += 8
        rangeDataPos = pos
...
plt.title('Spike Data From Signal')
